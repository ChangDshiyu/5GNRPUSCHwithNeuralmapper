...
class NeuralDemapper(Layer):
    def __init__(self, constellation, num_bits_per_symbol=4, dtype=tf.complex64):
        super().__init__()
        self.constellation = constellation  # 星座点
        self.num_bits_per_symbol = num_bits_per_symbol

        # 定义 Dense 层，用于计算 LLR
        self._dense_1 = Dense(128, activation='relu')
        self._dense_2 = Dense(128, activation='relu')
        self._dense_3 = Dense(num_bits_per_symbol, activation=None)  # 输出 LLR

    def call(self, inputs):
        x_hat, no_eff = inputs
        y = x_hat
        y = tf.reshape(y, [32, 2688, 1])
        no_eff = tf.reshape(no_eff,[32,2688,1])
        #('y',x_hat.shape)(128, 1, 14, 192, 1)
        #print('no',no_eff.shape)(128, 1, 14, 192, 1)
        # 保证 no_eff 的 dtype 与 x_hat 一致，转换为 tf.float32
        #no_eff = tf.cast(no_eff, dtype=tf.float32)
        #no_eff = tf.tile(no_eff, [1, 32])
        #no_eff = tf.cast(no_eff, dtype=tf.float32)
        #no_eff = tf.cast(no_eff, dtype=tf.float32)
        ...
        llr = self._dense_1(z)
        #print('llr1',llr.shape)
        llr = self._dense_2(llr)
        #print('llr2',llr.shape)
        llr = self._dense_3(llr)  # 输出 LLR
        llr = tf.reshape(llr,[32, 1, 14, 192, 4])
        print('llr3',llr.shape)
        #llr = tf.squeeze(llr, axis=-2)
        #print('llr3 (after squeeze)', llr)
